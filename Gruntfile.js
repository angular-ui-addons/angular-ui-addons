...
module.exports = function(grunt) {
  ...
  grunt.initConfig({
    ...
    dist: 'dist',
    mainModuleName: 'angular-ui-addons',
    templatesModuleName: '<%= mainModuleName %>.templates',
    filename: '<%= mainModuleName %>',
    filenamecustom: '<%= filename %>-custom',
    meta: {
      modules: 'angular.module("<%= mainModuleName %>", [<%= srcModules %>]);',
      tplmodules: 'angular.module("<%= templatesModuleName %>", [<%= tplModules %>]);',
      all: 'angular.module("<%= mainModuleName %>", ["<%= templatesModuleName %>", <%= srcModules %>]);',
      banner: ['/*',
               ' * <%= pkg.name %>',
               ' * <%= pkg.homepage %>\n',
...
//  grunt.registerTask('default', ['before-test', 'test', 'after-test']);
  ...
  //Common angular-ui-addons module containing all modules for src and templates
  ...
  function findModule(name) {
    ...
    var module = {
      name: name,
      moduleName: enquote('angular-ui-addons.' + name),
  ...
  function dependenciesForModule(name) {
    ...
    .forEach(function(contents) {
      ...
      dependencies.split(',').forEach(function(dep) {
        if (dep.indexOf('angular-ui-addons.') > -1) {
          var depName = dep.trim().replace('angular-ui-addons.','').replace(/['"]/g,'');
